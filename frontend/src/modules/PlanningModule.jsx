...
const ROOM_TYPES = ["living", "bedroom", "kitchen", "bathroom", "balcony", "utility", "common"];
...
export default function PlanningModule({ project, analysis, update, readOnly, projectId, setProject }) {
  ...
  const room = (t.rooms || []).find((r) => r.id === selectedRoom);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <Section title="Common areas & amenities" testid="common-area-section"
          actions={!readOnly && (
      ...
      <Section
        ...
        description="2D floor plate diagram. Click a room to edit its dimensions."
        testid="room-planning-section"
        actions={!readOnly && (
          <Button size="sm" variant="outline" className="h-7 rounded-sm text-xs" data-testid="add-room-button"
            onClick={() => setList("rooms", [...(t.rooms || []), { id: uid(), name: "New room", type: "bedroom", x: 0, y: 0, w: 3, h: 3 }])}>
            <Plus className="h-3 w-3 mr-1" /> Add room
          </Button>
        )}>
        <div className="grid lg:grid-cols-[1fr_320px] gap-4">
          <FloorPlate rooms={t.rooms || []} selectedId={selectedRoom} onSelect={setSelectedRoom} corridor={Number(t.corridor_width || 0)} />
          <div className="space-y-3">
            <div className="border border-slate-200 rounded-sm p-3">
              ...
              <div className="font-mono text-lg" data-testid="floor-room-area">{num(tm?.room_area_sqm, 2)} m²</div>
            </div>
            {room ? (
              <div className="border border-slate-200 rounded-sm p-3 space-y-2" data-testid="room-editor">
                <TextField label="Room name" value={room.name} disabled={readOnly} testid="room-name-input"
                  onChange={(v) => setList("rooms", t.rooms.map((r) => (r.id === room.id ? { ...r, name: v } : r)))} />
                <Select value={room.type} disabled={readOnly}
                  onValueChange={(v) => setList("rooms", t.rooms.map((r) => (r.id === room.id ? { ...r, type: v } : r)))}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[["w", "Width"], ["h", "Depth"], ["x", "X offset"], ["y", "Y offset"]].map(([k, label]) => (
                    <NumField key={k} label={label} suffix="m" step={0.1} value={room[k]} disabled={readOnly} testid={`room-${k}-input`}
                      onChange={(v) => setList("rooms", t.rooms.map((r) => (r.id === room.id ? { ...r, [k]: v } : r)))} />
                  ))}
                </div>
                ...
                {!readOnly && (
                  <Button size="sm" variant="outline" className="w-full rounded-sm text-xs text-red-600" data-testid="room-delete-button"
                    onClick={() => { setList("rooms", t.rooms.filter((r) => r.id !== room.id)); setSelectedRoom(null); }}>
