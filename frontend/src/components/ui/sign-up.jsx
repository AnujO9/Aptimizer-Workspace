 ...
 * Glass auth kit.
 *
 * Ported from the upstream TSX to JSX -- components.json sets `tsx: false`, so
 * this project has no TypeScript pipeline. Colours were retuned too: the source
 * leaned on Tailwind v4 tokens (`--color-primary`) and relative colour syntax
 * (`oklch(from …)`), neither of which resolves here. Our tokens are bare HSL
 * triplets, so every colour goes through `hsl(var(--token) / alpha)` instead.
 */
...
export const GlassButton = React.forwardRef(
  ({ className, children, size, contentClassName, onClick, ...props }, ref) => {
    // The visible surface is the inner <button>; clicks landing on the wrapper's
    // padding would otherwise be swallowed, so forward them.
...
// --- GLASS INPUT ---
/**
 * The pill-shaped field. `leading` and `trailing` are slots so each caller can
 * drive its own affordances (reveal toggle, submit arrow) without the field
 * knowing anything about the flow it sits in.
 */
export const GlassInput = React.forwardRef(({ leading, trailing, className, ...props }, ref) => (
...
// --- SHARED STYLES ---
/**
 * Render once per screen that uses the glass primitives. The effect is built
 * from `@property` angle interpolation and `:has()` state, neither of which
 * Tailwind can express.
 */
export const AuthGlassStyles = () => (
  <style>{`
    ...
    .glass-button { -webkit-tap-highlight-color: transparent; backdrop-filter: blur(clamp(1px, 0.125em, 4px)); transition: all var(--anim-time) var(--anim-ease); background: linear-gradient(-75deg, hsl(var(--background) / 5%), hsl(var(--background) / 20%), hsl(var(--background) / 5%)); box-shadow: inset 0 0.125em 0.125em hsl(var(--foreground) / 5%), inset 0 -0.125em 0.125em hsl(var(--background) / 50%), 0 0.25em 0.125em -0.125em hsl(var(--foreground) / 20%), 0 0 0.1em 0.25em inset hsl(var(--background) / 20%), 0 0 0 0 hsl(var(--background)); }
    .glass-button:hover:not(:disabled) { transform: scale(0.975); backdrop-filter: blur(0.01em); box-shadow: inset 0 0.125em 0.125em hsl(var(--foreground) / 5%), inset 0 -0.125em 0.125em hsl(var(--background) / 50%), 0 0.15em 0.05em -0.1em hsl(var(--foreground) / 25%), 0 0 0.05em 0.1em inset hsl(var(--background) / 50%), 0 0 0 0 hsl(var(--background)); }
    .glass-button:disabled { cursor: not-allowed; opacity: 0.5; }
    ...
    .glass-button-wrap:has(.glass-button:active) .glass-button { box-shadow: inset 0 0.125em 0.125em hsl(var(--foreground) / 5%), inset 0 -0.125em 0.125em hsl(var(--background) / 50%), 0 0.125em 0.125em -0.125em hsl(var(--foreground) / 20%), 0 0 0.1em 0.25em inset hsl(var(--background) / 20%), 0 0.225em 0.05em 0 hsl(var(--foreground) / 5%), 0 0.25em 0 0 hsl(var(--background) / 75%), inset 0 0.25em 0.05em 0 hsl(var(--foreground) / 15%); }
    ...
    .glass-input-wrap:focus-within .glass-input { backdrop-filter: blur(0.01em); box-shadow: inset 0 0.125em 0.125em hsl(var(--foreground) / 5%), inset 0 -0.125em 0.125em hsl(var(--background) / 50%), 0 0.15em 0.05em -0.1em hsl(var(--foreground) / 25%), 0 0 0.05em 0.1em inset hsl(var(--background) / 50%), 0 0 0 0 hsl(var(--background)); }
...
/**
 * Status dialog. Hoisted out of `AuthComponent`'s body -- defined inline it was
 * a fresh component type on every keystroke, so AnimatePresence tore the dialog
 * down and rebuilt it mid-animation.
 */
const StatusModal = ({ status, errorMessage, onClose }) => (
  <AnimatePresence>
    {status !== "closed" && (
      <motion.div
        initial={{ opacity: 0 }}
...
export const AuthComponent = ({ logo = <DefaultLogo />, brandName = "EaseMize" }) => {
  ...
  const handleFinalSubmit = (e) => {
    ...
    if (password !== confirmPassword) {
      ...
      setTimeout(() => setModalStatus("success"), totalDuration);
    }
  };
